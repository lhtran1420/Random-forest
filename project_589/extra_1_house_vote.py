...
def buildDecisionTree(train_index, depth):
    ...
    for i in range(0, len(attribute), 1):
        ...
        res = 0
        if first_choice > 0:
            res += first_choice / len(train_index) * Gini(nums00 / first_choice, (first_choice - nums00) / first_choice)

        if second_choice > 0:
            res += second_choice / len(train_index) * Gini(nums01 / second_choice,
                                                         (second_choice - nums01) / second_choice)

        if third_choice > 0:
            res += third_choice / len(train_index) * Gini(nums02 / third_choice, (third_choice - nums02) / third_choice)
