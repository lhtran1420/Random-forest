...
data = genfromtxt('hw3_wine.csv', skip_header=1)
def In(a, b, c):
    res = 0
    if a != 0:
...
def buildDecisionTree(train_index, attribute):
    ...
    for i in range(0, len(train_index), 1):
        if data[train_index[i]][0] == 1:
            first_class += 1
        elif data[train_index[i]][0] == 2:
            second_class += 1
        elif data[train_index[i]][0] == 3:
            third_class += 1
    ...
    if len(dt) == 1:
        x = dt[0][0]
        if x == 1:
            node = TreeNode(-1, 1000)
        elif x == 2:
            node = TreeNode(-1, 2000)
        else:
            node = TreeNode(-1, 3000)
        return node
    ...
    for i in range(0, len(attribute), 1):
        ...
        for j in range(0, len(a) - 1, 1):
            ...
            res = 0
            res = res + ((j + 1) / len(a) * In(nums1 / (j + 1), nums2 / (j + 1), nums3 / (j + 1)))
            length = len(a) - j - 1
            res = res + (length / len(a) * In((first_class - nums1) / length, (second_class - nums2) / length,
                                              (third_class - nums3) / length))
...
def performance(train_index, test_index, ntree):
    ...
    for i in range(0, ntree, 1):
        attribute = np.random.choice(13, 4, replace=False)
    ...
    accura = (true_pos1 + true_pos2 + true_pos3) / len(test_index)
    precision = (true_pos1 / (true_pos1 + false_pos1) + true_pos2 / (true_pos2 + false_pos2) + true_pos3 / (
                true_pos3 + false_pos3)) / 3
...
for i in range(0, len(ls), 1):
    ...
    accuracy, precision, recall, F1_score = performance(train_index, ls[i], 1)
    accura1 = accura1 + accuracy
    pre1 = pre1 + precision
    rec1 = rec1 + recall
    F1 = F1 + F1_score

    accuracy5, precision5, recall5, F15 = performance(train_index, ls[i], 5)
    accura5 = accura5 + accuracy5
    pre5 = pre5 + precision5
    rec5 = rec5 + recall5
    F5 = F5 + F15

    accuracy10, precision10, recall10, F110 = performance(train_index, ls[i], 10)
    accura10 = accura10 + accuracy10
    pre10 = pre10 + precision10
    rec10 = rec10 + recall10
    F10 = F10 + F110

    accuracy20, precision20, recall20, F120 = performance(train_index, ls[i], 20)
    accura20 = accura20 + accuracy20
    pre20 = pre20 + precision20
    rec20 = rec20 + recall20
    F20 = F20 + F120

    accuracy30, precision30, recall30, F130 = performance(train_index, ls[i], 30)
    accura30 = accura30 + accuracy30
    pre30 = pre30 + precision30
    rec30 = rec30 + recall30
    F30 = F30 + F130

    accuracy40, precision40, recall40, F140 = performance(train_index, ls[i], 40)
    accura40 = accura40 + accuracy40
    pre40 = pre40 + precision40
    rec40 = rec40 + recall40
    F40 = F40 + F140

    accuracy50, precision50, recall50, F150 = performance(train_index, ls[i], 50)
